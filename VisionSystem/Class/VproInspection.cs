...
public delegate void InspectionDataHandeler(string[] strValue);
public delegate void InspectionGrahphicHandler(List<CogPointMarker> cogPoint, List<CogLine> cogLine, List<CogCircle> cogCircle, List<CogCompositeShape> cogShape, List<CogEllipse> cogEllipse, List<ICogRegion> cogRegion, List<CogCompositeShape> cogBlobShape, string[] strData);
public delegate void JobLoadHandler(string strRes);
public delegate void MessageHandler(string strMsg, Color color, bool bShoPopup, bool bError, string Type);
public delegate void InspectionRegionGraphicHandler(ICogGraphic[] _Region);
public class VproInspection
{
    public InspectionDataHandeler _OnInspData;
    public InspectionGrahphicHandler _OnInspGraphic;
    public InspectionRegionGraphicHandler _OnInspRegionGraphic;
    ...
    double[] dMasPosX = new double[4] { 314.62746611378094, 913.16169441558316, 1506.9046111834791, 2131.7559884404222 };
    double dMasPosY = 905.235;
    double dResolution = 0.374;
    double dResY = 0.741;
    ...
    public void VJobrun(int nSel, ICogImage cogGrab, GlovalVar.GraphicParam[] graphicParam, GlovalVar.ModelParam ModelParam)
    {
        ...
        try
        {
            ...
            if (_job != null && cogImg != null)
            {
                ...
                {
                    if (_Model.bPinChange)
                    {
                        if (_DataAnalysisTool.Results.AggregatePass)
                            strData[0] = "1";
                        else
                            strData[0] = "1";
                    }
                    ...
                    if (_Model.bBCRInsp)
                    {
                        for (int i = 0; i < _job.Tools.Count; i++)
                        {
                            if (_job.Tools[i].GetType() == _cogIDTool.GetType())
                            {
                                if ((_job.Tools[i] as CogIDTool).Results.Count != 0)
                                {
                                    strData[1] += (_job.Tools[i] as CogIDTool).Results[0].DecodedData.DecodedString;
                                }
                                else
                                    strData[1] += "Err ID";
                            }
                            if (_job.Tools[i].GetType() == _cogOCRMaxTool.GetType())
                            {
                                if (((_job.Tools[i] as CogOCRMaxTool).LineResult != null))
                                {
                                    strData[1] += (_job.Tools[i] as CogOCRMaxTool).LineResult.ResultString;
                                }
                                else
                                    strData[1] += "Err OCR";
                            }
                    ...
                    #region 로봇 좌표 이름이 XYZ 시
                    if (_Model.bAlingInsp)
                    {
                        if (_DataAnalysisTool.RunParams.Count >= 3)
                        {
                            if (_DataAnalysisTool.RunParams[0].Name == "X")
                            {
                                strData[2] = _DataAnalysisTool.RunParams[0].CurrentValue.ToString();
                            }
                            if (_DataAnalysisTool.RunParams[1].Name == "Y")
                            {
                                strData[3] = _DataAnalysisTool.RunParams[1].CurrentValue.ToString();
                            }
                            if (_DataAnalysisTool.RunParams[2].Name == "Z")
                            {
                                strData[4] = _DataAnalysisTool.RunParams[2].CurrentValue.ToString();
                            }
                        }
                    }
                    else
                    ...
                    if (_Model.bPinChange)
                    {
                        for (int j = 0; j < _DataAnalysisTool.RunParams.Count; j++)
                        {
                            if (_DataAnalysisTool.Results[j].ToleranceStatus == CogDataAnalysisToleranceStatusConstants.Pass)
                                strData[5] = "1" + strData[5];
                            else
                                strData[5] = "0" + strData[5];
                        }
                    ...
                    #region 개별 결과 Bit 표현
                    if (_Model.bDefectInsp)
                    {
                        for (int j = 0; j < _DataAnalysisTool.RunParams.Count; j++)
                        {
                            if (_DataAnalysisTool.Results[j].ToleranceStatus == CogDataAnalysisToleranceStatusConstants.Pass)
                                strData[6] = "1" + strData[6];
                            else
                                strData[6] = "0" + strData[6];
                        }
                    }
                    else
                    {
                ...
                {
                    _OnMessage("DataAnalsisTool is Error", Color.Red, false, false, "alarm");
                    strData[0] = "2";
                    strData[1] = "";
                    strData[2] = "";
                    strData[3] = "";
                ...
                if (_OnInspGraphic != null)
                    _OnInspGraphic(cogPoint, cogLine, cogCircle, cogShape, cogEllipse, cogRegion, cogBlobShape, strData);
            ...
            {
                if (_job == null)
                    _OnMessage("The working file was not loaded.", Color.Red, false, false, "alarm");

                if (cogImg == null)
                    _OnMessage("No Image", Color.Red, false, false, "alarm");
            }
        }
    ...
    private string[] GetResultData(int nCnt, double[] dPosX, double[] dPosY, double[] dRotation, int[] nFind, ref bool bRes)
    {
        var strData = new string[6] { "", "", "", "", "", "" };
        var dLocationX = new double[4] { 9999.9, 9999.9, 9999.9, 9999.9 };
        var dLocationY = new double[4] { 0, 0, 0, 0 };
        var dRotations = new double[4] { 0, 0, 0, 0 };
        var dAlignX = new double[4] { 0.0, 0.0, 0.0, 0.0 };
        var dAlignZ = new double[4] { 0.0, 0.0, 0.0, 0.0 };

        for (int i = 0; i < 4; i++)
        {
            var nIdx = GetIndex(dPosX[i]);

            if (nIdx == 1 || nIdx == 2)
            {
                if (dLocationX[nIdx] == 9999.9)
                {
                    dLocationX[nIdx] = dPosX[i];
                    dLocationY[nIdx] = dPosY[i];
                    dRotation[i] = dRotation[i];
                }
                else
                {
                    var dTemp1 = dLocationX[nIdx] - dMasPosX[nIdx];
                    var dTemp2 = dPosX[i] - dMasPosX[nIdx];

                    if (Math.Abs(dTemp1) > Math.Abs(dTemp2))
                    {
                        dLocationX[nIdx] = dPosX[i];
                        dRotation[i] = dRotation[i];
                    }

                    dTemp1 = dLocationY[nIdx] - dMasPosY;
                    dTemp2 = dPosY[i] - dMasPosY;
        ...
        for (var i = 0; i < 4; i++)
        {
            dAlignX[i] = dMasPosX[i] - dLocationX[i];
            dAlignX[i] = dAlignX[i] * dResolution * -0.1;

            dAlignZ[i] = dMasPosY - dLocationY[i];
            dAlignZ[i] = dAlignZ[i] * dResY;

            if (i == 1)
            {
                if (dLocationX[i] != 9999.9)
                    nFind[i] = 0;
                else
                    nFind[i] = 1;
            }
        ...
        var dTempX = dAlignX[0];

        for (var i = 1; i < 4; i++)
        {
            if (Math.Abs(dTempX) > Math.Abs(dAlignX[i]))
                dTempX = dAlignX[i];
        }

        var dTempZ = dAlignZ[0];

        for (var i = 1; i < 4; i++)
        {
            if (Math.Abs(dTempZ) > Math.Abs(dAlignZ[i]))
                dTempZ = dAlignZ[i];
        }

        if (Math.Abs(dTempX) > 30)
            bRes = false;

        if (nFind[1] == 1 && nFind[2] == 1)
            bRes = false;

        strData[2] = string.Format("{0},{1}", dTempX, dTempZ);
        strData[3] = dMasPosY.ToString();
        //strData[4] = string.Format("{0},{1}", dRotation[0], dRotation[1]);

        return strData;
    }

    private int GetIndex(double dPosX)
    {
        var nIdx = -1;

        for (var i = 0; i < 4; i++)
        {
            if ((dPosX > dMasPosX[i] - 200) && (dPosX < dMasPosX[i] + 200))
            {
                nIdx = i;
                break;
            }
